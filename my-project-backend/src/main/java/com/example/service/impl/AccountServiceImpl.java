...
public class AccountServiceImpl extends ServiceImpl<AccountMapper, Account> implements AccountService {
    ...
    public String registerEmailAccount(EmailRegisterVO info) {
        ...
        Account account = new Account(null, info.getUsername(),
                password, email, Const.ROLE_DEFAULT, null, LocalDateTime.now(), false, false);
    ...
    public String modifyEmail(int id, ModifyEmailVO vo) {
        ...
        if (code == null) return "请先获取验证码";
        if (!code.equals(vo.getCode())) return "验证码错误，i请重新输入";

        this.deleteEmailVerifyCode(email);
        Account account = this.findAccountByNameOrEmail(email);

        if (account != null && account.getId() != id) return "该电子邮件已被其他账号绑定，无法完成此操作！";
